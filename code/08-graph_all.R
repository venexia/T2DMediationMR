...
for (out in c("cad","pad")) {
  ...
  df <- results[results$outcome==out,]
  df <- merge(features, df, by.x = "trait", by.y = "feature")
  ...
  df$direct <- ifelse(df$direct==TRUE,"Direct","Indirect")
  ...
  ggsave(paste0("output/T2DSubtypesMR_",out,"_all_features.pdf"), width = 210, height = 297, units = "mm")
  ...
  ggsave(paste0("output/T2DSubtypesMR_",out,"_all_t2d.pdf"), width = 210, height = 297, units = "mm")
