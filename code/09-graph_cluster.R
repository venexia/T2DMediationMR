...
for (out in c("cad","pad")) {
  ...
  results_tmp <- results[results$outcome==out,]
  
  df <- merge(df, results_tmp, by.x = "trait", by.y = "feature", all.x = TRUE)
  ...
  tmp <- df[df$analysis=="MVMR" & df$t2d==FALSE & df$direct=="Indirect",]
  ...
  ggsave(paste0("output/T2DSubtypesMR_",out,"_cluster_features.pdf"), width = 210, height = 297, units = "mm")
